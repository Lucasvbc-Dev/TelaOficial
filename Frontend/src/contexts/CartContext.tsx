...
export const CartProvider = ({ children }: { children: ReactNode }) => {
  ...
  const removeItem = (id: string, size: CartItem["size"]) => {
    setItems((prev) => prev.filter((i) => !(i.id === id && i.size === size)));
  };

  const updateQuantity = (id: string, size: CartItem["size"], quantity: number) => {
    if (quantity <= 0) {
      removeItem(id, size);
      return;
    }
  ...
  return (
    <CartContext.Provider
      value={{ items, addItem, removeItem, updateQuantity, clearCart, totalItems, isCartOpen, setIsCartOpen }}
    >
      {children}
    </CartContext.Provider>
  );
